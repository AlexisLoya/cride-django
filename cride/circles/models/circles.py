...
class Circle(CRideModel):
    ...
    by itd members. To join a circle a user must recive an unique
    invitation code from an existing circle members.
    """
    name = models.CharField('circle name', max_length=140)

    slug_name = models.SlugField(unique=True, max_length=40)

    about = models.CharField('circle desscription', max_length=255)
    ...
    members_limit = models.PositiveIntegerField(
        default=0,
        help_text='If circle is limited, the will be the limit on the number of members.'
    )
